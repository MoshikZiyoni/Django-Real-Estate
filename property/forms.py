...
class SearchingForm(forms.Form):
   ...
   location = forms.CharField(label='location',widget=forms.TextInput(attrs={'placeholder': 'תל אביב יפו/רמת גן'}))
   neighbourhood=forms.CharField(label="neighbourhood")
   ...
   def __str__(self):
      return self.room +","+ self.floor+","+ self.property_type+","+ self.location+","+ self.neighbourhood+","+ self.price+","+self.investment_range
   def __repr__(self):
         return self.__str__() 

   def get_form(self):
      return self.data.get('room'), self.data.get('floor'),self.data.get('property_type'),self.data.get('location'),self.data.get('neighbourhood'),self.data.get('price'),self.data.get('investment_range')
