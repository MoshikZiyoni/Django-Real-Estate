...
def result(request):
    ...
    found_apt = Property.objects.filter(room__lte=new_form[0], floor__lte=new_form[1], property_type__contains=new_form[2],
                                        location__contains=new_form[3], neighbourhood__contains=new_form[4], price__lte=new_form[5]).values()
    if found_apt:
        ...
        for apt in found_apt:
            ...
            found_project = Project.objects.filter(
                neighbourhood__contains=apt['neighbourhood']).values()
            # print (found_project)
...
def single_apt(request, id):
    ...
    if int(futureprice) in range(1, 11):
        ...
        found_project = Project.objects.filter(
            neighbourhood=property.neighbourhood).values()
...
def nav_search(request):
    if request.method == "POST":
        ...
        A_property = Property.objects.filter(Q(location__contains=searched) | Q(street__contains=searched) | Q(room__contains=searched) | Q(
            floor__contains=searched) | Q(property_type__contains=searched) | Q(price__contains=searched) | Q(neighbourhood__contains=searched))
        B_projects = Project.objects.filter(Q(type_project__contains=searched) | Q(size_project__contains=searched) | Q(company__contains=searched) | Q(
            value__contains=searched) | Q(location__contains=searched) | Q(dates__contains=searched) | Q(neighbourhood__contains=searched))
    if searched == None:
        searched = "nothing"
